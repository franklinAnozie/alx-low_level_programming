...
dog_t *new_dog(char *name, float age, char *owner)
{
	...
	for (owner_length = 0; owner[owner_length] != '\0';)
		owner_length++;
	...
	another_dog->owner = malloc((owner_length + 1) * sizeof(another_dog->owner));
	...
	another_dog->name = malloc((name_length + 1) * sizeof(another_dog->name));

	if (!another_dog->name)
	{
		free(another_dog->owner);
		free(another_dog);
		return (NULL);
	}
